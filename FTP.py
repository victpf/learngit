...
class MYFTP:
    def __init__(self, hostname, port, username, password,remote_dir):
...
#            print(list1)
            ...
            for j in range(len(self.filelist)):
                self.download_file(self.filelist[j],self.filelist[j])
...
#    file = open('C:\tpf\log.txt','a')
    ...
    password='cw819142'
    remote_dir = 'TPF'
    f = MYFTP(hostname, port, username, password,remote_dir)
    f.connftp()
